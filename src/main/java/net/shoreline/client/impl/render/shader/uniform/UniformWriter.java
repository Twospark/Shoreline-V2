...
public class UniformWriter
{
    ...
    public void setUniforms(PostChain chain, UniformBuilder uniformBuilder)
    {
        Map<String, List<UniformValue>> configs = uniformBuilder.getConfig();
        if (configs.isEmpty())
        {
            return;
        }

        List<PostPass> passes = ((IPostChain) chain).shoreline$getPasses();
        for (PostPass pass : passes)
        {
            Map<String, GpuBuffer> uniformBuffers = ((IPostPass) pass).shoreline$getUniformBuffers();
            for (Map.Entry<String, List<UniformValue>> entry : configs.entrySet())
            {
                ...
                int handle    = ((IGLBuffer) dest).shoreline$getHandle();
