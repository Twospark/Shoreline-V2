...
public class UnboundAnimation
{
    ...
    public float get()
    {
        return (float) Mth.lerp(easing.ease(toDelta(lastMillis, length)), prev, target);
    }

    public float get(float target)
    {
        float lerp = (float) Mth.lerp(easing.ease(toDelta(lastMillis, length)), prev, this.target);
    ...
    {
        return Math.clamp(toDelta(start) / (float) length, 0.0f, 1.0f);
    }
