...
public class TrapPositionCalc
{
    ...
    public void calcTrap(AABB boundingBox, TrapSpec trapSpec)
    {
        trapPositions.clear();

        Set<BlockPos> origin = BlockPos.betweenClosedStream(boundingBox)
                               ...
                               int bodyY)
    {
        ...
        if (feetY == y)
        {
            ...
            {
                trapPositions.put(blockPos.below(), TrapLayer.FLOOR);
            }

            if (layers.contains(TrapLayer.CEILING))
            {
                trapPositions.put(blockPos.above(2), TrapLayer.CEILING);
            }
        }
